...
void WireframeRenderer::renderScene(Scene& s, int width, int height, PixelSurface& output)
{
    ...
    for (WireframeElement e : stuff)
    {
        for (uint i = 0; i < e.points.size(); ++i)
        {
            ...
            Linedraw(x1, y1, x2, y2, vec3(160, 160, 160), output);
        }
    }
    
    // draw vertex points, vertex normals and/or centroid normal, draw after drawing wires to ensure
    // these are drawn "on top" of wires (and not the other way round)
    for (WireframeElement e : stuff)
    {
        for (uint i = 0; i < e.points.size(); ++i)
        {
            ...
            if (opt_ShowCentroidNormals && e.hasCentroid)
            {
    ...
    for (Light* light : s.getLights())
    {
        ...
        if (left.z < 0.1f || right.z < 0.1f || top.z < 0.1f || bot.z < 0.1f || front.z < 0.1f || back.z < 0.1f)
        {
            continue;
        }
...
list<Renderer::Option> WireframeRenderer::getOptions()
{
    list<Option> options;
    
    options.push_back(Option("ShowVertexNormals", opt_ShowVertexNormals));
    options.push_back(Option("ShowCentroidNormals", opt_ShowCentroidNormals));
    
    return options;
}

void WireframeRenderer::setOptions(list<Renderer::Option> opts)
{
    for (Option opt : opts)
    {
        setOption(opt);
...
void WireframeRenderer::setOption(Option opt)
{
    if (opt.name == "ShowVertexNormals")
    {
        opt_ShowVertexNormals = opt.boolval;
    }
    else if (opt.name == "ShowCentroidNormals")
    {
        opt_ShowCentroidNormals = opt.boolval;
    }
}
