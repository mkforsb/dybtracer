 ...
 */
bool RayTracingRenderer::cast(RayTracingRenderer* renderer, const Scene* s, const list<Object*>& objects, const list<Light*>& lights, vec3 ray_org, vec3 ray_dir, vec3& color, int rayGeneration, bool enableCulling)
{
    ...
    if (closest.material.opacity != vec3(1,1,1))
    {
        ...
        if (closest.material.refraction != 1.0f)
        {
            ...
            if (k > 0)
            {
                ...
                cast(renderer, s, objects, lights, closest.position + 0.001f * T, T, color_behind, rayGeneration + 1, false);
            }
            // }
        }
        else
        {
            ...
            cast(renderer, s, objects, lights, closest.position + 0.001f * ray_dir, ray_dir, color_behind, rayGeneration + 1, enableCulling);
        }
    ...
    if (any(closest.material.reflectivity))
    {
        ...
        cast(renderer, s, objects, lights, closest.position + 0.001f*re_reflect, re_reflect, color_reflect, rayGeneration + 1, false);
...
void* RayTracingRenderer::renderingThread(void* arg)
{
    ...
    for (int y = data.y_from; y < data.y_to; y += 1)
    {
        ...
        {
            vec3 ray = s->camera.getRotationMatrix() * vec3((double) x - data.width/2.0, (double) y - data.height/2.0, F);
            vec3 color;
            
            if (cast(data.renderer, s, objects, lights, campos, ray, color, 0, true))
            {
