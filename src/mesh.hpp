...
private:
     ...
     */
    bool getIntersection(vec3 start, vec3 dir, vec3 dir_inv, Intersection& is, bool enableCulling, BoundingBox* box);
    
    void flip(vec3 comps);
    
    vec3 m_position;
    
    void moveBVH(BoundingBox* box, vec3 position);
     ...
     */
    int m_numSetTemporaryVertices = 0;
    Vertex m_temporaryVertices[3];
    
    /**
     * Main bounding box which contains the entire mesh
     */
    BoundingBox* m_mainbox;
